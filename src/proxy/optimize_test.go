...
// +--------------+----------+----------+-------------------------------------------------------------------+
var (
	optimizeTableResult = &sqltypes.Result{
		RowsAffected: 1,
		...
		Rows: [][]sqltypes.Value{
			{
				...
				sqltypes.MakeTrusted(querypb.Type_VARCHAR, []byte("optimize")),
				sqltypes.MakeTrusted(querypb.Type_VARCHAR, []byte("note")),
				sqltypes.MakeTrusted(querypb.Type_VARCHAR, []byte("Table does not support optimize, doing recreate + analyze instead")),
			},
			{
				sqltypes.MakeTrusted(querypb.Type_VARCHAR, []byte("test.t")),
				sqltypes.MakeTrusted(querypb.Type_VARCHAR, []byte("optimize")),
...
func TestProxyOptimizeTable(t *testing.T) {
	...
	// fakedbs.
	{
		...
		fakedbs.AddQueryPattern("optimize .*", optimizeTableResult)
	}
