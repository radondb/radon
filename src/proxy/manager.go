...
func (mgr *Manager) killIdleTxn() error {
	...
	for _, tsi := range ssInTxn {
		...
		if tsi.Time > mgr.idleTimeout && (tsi.Info == sqlparser.BeginTxnStr || tsi.Info == "") {
			log.Warning("the session in transaction will be killed, the session info is: %v:", tsi)
			str := fmt.Sprintf("the session in transaction is idle for a long time: %d s > %d s.", tsi.Time, mgr.idleTimeout)
			ss.Kill(tsi.ID, str)
		}
	}
	return nil
...
func NewManager(log *xlog.Log, sessions *Sessions, conf *config.ProxyConfig) *Manager {
	return &Manager{
		log:         log,
		sessions:    sessions,
		idleTimeout: conf.IdleTxnTimeout,
		done:        make(chan bool),
