...
func TestJoinExecutor(t *testing.T) {
	...
	querys := []string{
		...
		"select A.id, B.name from B join A on A.name=B.name where A.id = 3 and B.id=1 group by A.id",
		"select A.id, B.name, B.id, A.name from B left join A on A.id>B.id and A.name!=B.name where A.id = 2 group by A.id",
		"select A.id, B.name, B.id, A.name from A left join B on A.name=B.name and A.id>=B.id and A.name<=>B.name and A.id >2 where B.name is null group by A.id",
