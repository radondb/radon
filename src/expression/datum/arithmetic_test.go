...
func TestAdd(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			v1:     NewDNull(true),
			v2:     NewDInt(1, false),
			field:  &IField{IntResult, 0, 0, false, false, 63},
			resTyp: TypeNull,
			resStr: "NULL",
		},
		{
		...
		},
		{
			...
			v1:    NewDInt(-1, true),
			v2:    NewDInt(2, true),
			field: &IField{IntResult, 0, 0, true, false, 63},
		...
		},
		{
			...
			field:  &IField{DecimalResult, 0, 2, false, false, 63},
			resTyp: TypeDecimal,
			resStr: "4",
		},
		{
		...
		},
		{
			v1:     NewDFloat(1.23),
			v2:     NewDDecimal(decimal.NewFromFloat(2.77)),
			field:  &IField{RealResult, 0, NotFixedDec, false, false, 63},
			resTyp: TypeFloat,
			resStr: "4",
		},
		{
...
func TestMul(t *testing.T) {
	tcases := []struct {
...

		{
			v1:     NewDDecimal(decimal.NewFromFloat(1.23)),
			v2:     NewDDecimal(decimal.NewFromFloat(2.77)),
			field:  &IField{DecimalResult, 0, 4, false, false, 63},
			resTyp: TypeDecimal,
			resStr: "3.4071",
		},
		{
			...
			v1:    NewDInt(2, true),
			v2:    NewDDecimal(decimal.NewFromFloat(math.MaxFloat64)),
			field: &IField{DecimalResult, 0, 0, false, false, 63},
...
func TestDiv(t *testing.T) {
	tcases := []struct {
		...
		},
		{
			v1:     NewDInt(2, true),
			v2:     NewDInt(0, false),
			field:  &IField{DecimalResult, 0, 0, true, false, 63},
			resTyp: TypeNull,
			resStr: "NULL",
		},
		{
		...
		{
			v1:    NewDFloat(math.MaxFloat64),
			v2:    NewDString("0.5", 10, 33),
			field: &IField{RealResult, 0, 0, true, false, 63},
...
func TestIntDiv(t *testing.T) {
	tcases := []struct {
		...
		},
		{
			v1:     NewDFloat(1),
			v2:     NewDInt(2, false),
			field:  &IField{IntResult, 0, NotFixedDec, false, false, 63},
			resTyp: TypeInt,
			resStr: "0",
		},
		{
