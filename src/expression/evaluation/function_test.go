...
func TestFunc(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			field: &datum.IField{
				ResTyp:  datum.StringResult,
				Charset: 63,
				Scale:   31,
			},
			saved: datum.NewDString("1", 10, 63),
		},
		{
			name:     "if (a > con1, a, d)",
			funcName: "if",
		...
		},
		{
			...
			field: &datum.IField{
				ResTyp:  datum.TimeResult,
				Charset: 63,
				Scale:   3,
			},
			saved: datum.NewDTime(querypb.Type_DATETIME, 3, 2020, 8, 21, 8, 0, 0, 230000),
		},
		{
			name:     "if (s>z,s,z)",
			funcName: "if",
