...
func (txn *Txn) executeXA(req *xcontext.RequestContext, state txnXAState) error {
	...
	oneShard := func(state txnXAState, back string, txn *Txn, query string) {
		...
		case txnXAStateCommit, txnXAStateRollback:
			...
			if state == txnXAStateCommit && x != nil {
				if err := txn.WriteXaLog(back); err != nil {
					log.Error("txn.xa.WriteXaLog[maxretry:%v, retried:%v].state[%v].on[%v].query[%v].error[%T]:%+v",
						maxRetry, maxRetry, state, c.Address(), query, x, x)
				}
			}
		}
...
func (txn *Txn) WriteXaLog(backend string) error {
	return txn.mgr.xaCheck.WriteXaLog(txn, backend)
}
