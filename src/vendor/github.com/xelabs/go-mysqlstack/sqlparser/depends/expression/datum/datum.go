...
func DatumToVal(d Datum, f *IField) (v sqltypes.Value, err error) {
	switch f.Type {
	case StringResult:
		v = sqltypes.MakeTrusted(sqltypes.VarBinary, []byte(d.ValStr()))
	case IntResult:
		res, flag := d.ValInt()
		if flag {
			v = sqltypes.MakeTrusted(sqltypes.Uint64, strconv.AppendUint(nil, uint64(res), 10))
		} else {
			v = sqltypes.MakeTrusted(sqltypes.Int64, strconv.AppendInt(nil, res, 10))
		}
	case DecimalResult:
		v = sqltypes.MakeTrusted(sqltypes.Decimal, []byte(d.ValDecimal().String()))
	case RealResult:
		v = sqltypes.MakeTrusted(sqltypes.Float64, strconv.AppendFloat(nil, d.ValReal(), 'f', -1, 64))
	case RowResult:
		return v, errors.Errorf("unsupport type: RowResult")
	case TimeResult:
		typ := sqltypes.Datetime
		if t, ok := d.(*DTime); ok {
			typ = t.typ
		}
		sqltypes.MakeTrusted(typ, []byte(d.ValStr()))
	case DurationResult:
		sqltypes.MakeTrusted(sqltypes.Time, []byte(d.ValStr()))
	}
	return v, nil
}
