...
func (shift *Shift) setRadonReadOnly(v bool) error {
	...
	if resp == nil || resp.StatusCode != http.StatusOK {
		return fmt.Errorf("shift.set.radon.readonly[%s].response.error:%+s", path, xbase.HTTPReadBody(resp))
	}
	return nil
}
...
func (shift *Shift) setRadonRule() error {
	...
	if resp == nil || resp.StatusCode != http.StatusOK {
		return fmt.Errorf("shift.set.radon.shard.rule[%s].response.error:%+s", path, xbase.HTTPReadBody(resp))
	}
	return nil
}
...
func (shift *Shift) setRadonThrottle(factor float32) error {
	...
	if resp == nil || resp.StatusCode != http.StatusOK {
		return fmt.Errorf("shift.set.radon.throttle[%s].response.error:%+s", path, xbase.HTTPReadBody(resp))
	}
	return nil
}
