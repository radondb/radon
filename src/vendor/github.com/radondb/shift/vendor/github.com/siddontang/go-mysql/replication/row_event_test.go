...
func TestDecodeDecimal(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		assert.Equal(t, expectedFloat, value.(float64))
		...
		assert.Equal(t, expectedDecimal, value.(decimal.Decimal))
		assert.Equal(t, tc.ExpectedPos, pos)
		assert.Equal(t, tc.ExpectedErr, err)
	}
}
...
func TestParseRowPanic(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, int32(16270), rows.Rows[0][0])
}
...
func TestParseJson(t *testing.T) {
	...
	for _, tbl := range tbls {
		...
		err = rows.Decode(tbl)
		assert.Nil(t, err)
		assert.Equal(t, float64(1), rows.Rows[0][1])
	}
	...
	for _, ltbl := range longTbls {
		...
		assert.Nil(t, err)
		assert.Equal(t, float64(101), rows.Rows[0][1])
	}
}
func TestParseJsonDecimal(t *testing.T) {
	...
	for _, tbl := range tbls {
		...
		err = rows.Decode(tbl)
		assert.Nil(t, err)
		assert.Equal(t, decimal.NewFromFloat(1), rows.Rows[0][1])
	}
	...
	for _, ltbl := range longTbls {
		...
		err = rows.Decode(ltbl)
		assert.Nil(t, err)
		assert.Equal(t, decimal.NewFromFloat(101), rows.Rows[0][1])
	}
}
...
func TestEnum(t *testing.T) {
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.Equal(t, int64(1), rows.Rows[0][1])
}
...
func TestMultiBytesEnum(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, int64(1), rows.Rows[0][1])
	assert.Equal(t, int64(257), rows.Rows[1][1])
}
...
func TestSet(t *testing.T) {
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.Equal(t, int64(5), rows.Rows[0][1])
}
...
func TestJsonCompatibility(t *testing.T) {
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.EqualValues(t, []uint8("{}"), rows.Rows[0][2])
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.EqualValues(t, []uint8("{}"), rows.Rows[1][2])
	assert.EqualValues(t, []uint8("{\"a\":1234}"), rows.Rows[2][2])
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.EqualValues(t, []uint8("{\"a\":1234}"), rows.Rows[1][2])
	assert.EqualValues(t, []uint8("{}"), rows.Rows[2][2])
	...
	err = rows.Decode(data)
	assert.Nil(t, err)
	assert.EqualValues(t, []uint8("null"), rows.Rows[1][2])
	assert.EqualValues(t, []uint8("{\"a\":1234}"), rows.Rows[2][2])
