...
func shardBalanceAdviceHandler(log *xlog.Log, proxy *proxy.Proxy, w rest.ResponseWriter, r *rest.Request) {
	...
	// Find the max and min backend.
	found := false
	var max, min backendSize
	var tableSize float64
	var database, table string
	// Find the global table that need to be migrated.
	...
	// Find the partition table that need to be migrated.
	if !found {
	...
	// Try to sync all databases from max.databases to min.databases.
